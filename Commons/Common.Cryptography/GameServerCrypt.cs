...
public class GameServerCrypt : INetworkCrypt
{
    ...
    // Вторая часть ключа по всей видимости статична
    private static readonly byte[] LastHalfBlowfish = 
        [0xc8, 0x27, 0x93, 0x01, 0xa1, 0x6c, 0x31, 0x97]; 

    public GameServerCrypt()
    {
        var firstHalfBlowfish = new byte[8];
        
        //TODO: рассмотреть возможность использовать пулл ключей
        Random.Shared.NextBytes(firstHalfBlowfish);

        key = [..firstHalfBlowfish, ..LastHalfBlowfish];
